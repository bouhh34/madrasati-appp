...
function navigate(page){qsa(".page").forEach(p=>p.classList.toggle("active",p.id===`page-${page}`));qsa("[data-page]").forEach(b=>b.classList.toggle("active",b.dataset.page===page));const titles={home:"الرئيسية",academic:"الأقسام والنتائج",children:"أبنائي",admin:"إدارة المدرسة",security:"الأمان والسجل",settings:"الإعدادات"};$("pageTitle").textContent=tr(titles[page]||"مدرستي");if(page==="admin")loadUsers();if(page==="security")loadAudit();if(page==="children")loadChildren();if(page==="settings")loadBranding()}
